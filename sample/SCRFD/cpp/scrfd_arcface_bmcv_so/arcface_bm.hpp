...
class ArcFaceBM {
 public:
    ...
    //static bm_image AlignBy5PtsRGB(bm_image& src_rgb, const cvai_pts_t& pts, int out_w = 112, int out_h = 112);
    int AlignBy5PtsBMI(bm_image& src_bmi, const cvai_pts_t& pts, bm_image& dst_rgb112);

    // 单张 112x112 RGB 前向（内部会走 bmcv_image_convert_to 到 [-1,1]、绑定 tensor、forward）
    //std::vector<float> Forward112RGB(cv::Mat& rgb112);
    std::vector<float> Forward112RGB(bm_image& rgb112);

    // BGR 输入也可（内部转换成 RGB 再走前向）
    //std::vector<float> Forward112BGR(const cv::Mat& bgr112);
    std::vector<float> Forward112BGR(bm_image& bgr112);
    ...
    bool arcface_inference(bm_image& input, const ScrfdBoxVec& detected_boxes, std::vector<arcface_data>& output);
...
private:
    ...
    bool UploadAndPreprocess(bm_image& rgb112, bm_image& out_converto);
