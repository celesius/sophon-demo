...
ARC_SRC_5PTS = np.array([[38.2946, 51.6963],
                         [73.5318, 51.5014],
                         [56.0252, 71.7366],
                         [41.5493, 92.3655],
...
def align_by_5pts(bgr_img: np.ndarray, kps: np.ndarray, out_size=(112, 112)) -> np.ndarray:
    """ kps: (5,2) [le,re,nose,lm,rm]; return RGB 112x112 """
    M = cv2.estimateAffinePartial2D(kps.astype(np.float32), ARC_SRC_5PTS, method=cv2.LMEDS)[0]
    rgb = cv2.cvtColor(bgr_img, cv2.COLOR_BGR2RGB)
    aligned = cv2.warpAffine(rgb, M, out_size)
    return aligned

def arcface_preprocess(rgb_112: np.ndarray) -> np.ndarray:
    x = rgb_112.astype(np.float32) / 127.5 - 1.0   # [-1,1]
    x = np.transpose(x, (2,0,1))[None, ...]        # (1,3,112,112)
    return x

class ArcFaceBM(object):
    """Simple ArcFace engine for bmodel on Sophon SAIL"""
    def __init__(self, bmodel_path: str, dev_id: int = 0):
        ...
        self.input_shape = self.net.get_input_shape(self.graph_name, self.input_name)  # [N,C,H,W]
        self.in_h = self.input_shape[2]; self.in_w = self.input_shape[3]
    
    def __call__(self, batch: np.ndarray) -> np.ndarray:
        # batch: (N,3,112,112), float32
        outputs = self.net.process(self.graph_name, {self.input_name: batch})
        feat = list(outputs.values())[0]
        # L2 normalize
        feat = feat / (np.linalg.norm(feat, axis=1, keepdims=True) + 1e-8)
        return feat

def build_gallery_from_dir(detector, arc_engine: ArcFaceBM, gallery_dir: str,
                           det_conf: float = 0.5) -> Tuple[np.ndarray, List[str]]:
    """Build gallery features and names from a directory structure:
       gallery_dir/personA/*.jpg, gallery_dir/personB/*.png, ...
       Returns (features: [M,512], names: [M])
    """
    feats = []
    names = []
    for person in sorted(os.listdir(gallery_dir)):
        ...
        for ext in ('*.jpg','*.png','*.jpeg','*.bmp','*.webp'):
            img_paths.extend(glob.glob(os.path.join(pdir, ext)))
        person_feats = []
        for fp in sorted(img_paths):
            img = cv2.imdecode(np.fromfile(fp, dtype=np.uint8), -1)
            if img is None: 
                continue
            dets = detector([img])[0]  # Nx(…)
            if dets.shape[0] == 0: 
                continue
            # choose biggest face
            areas = (dets[:,2]-dets[:,0])*(dets[:,3]-dets[:,1])
            i = int(np.argmax(areas))
            if dets[i, -2] < det_conf:
                continue
            # keypoints present?
            kps = None
            if dets.shape[1] >= 4+10+2:  # bbox(4)+kps(10)+score+class
                kps = dets[i, 4:14].reshape(5,2)
            # align or center crop
            if kps is not None:
                rgb = align_by_5pts(img, kps)
            else:
                x1,y1,x2,y2 = dets[i,:4].astype(int)
                face = img[max(0,y1):max(0,y2), max(0,x1):max(0,x2)]
                if face.size==0: 
                    continue
                rgb = cv2.cvtColor(cv2.resize(face, (112,112)), cv2.COLOR_BGR2RGB)
            inp = arcface_preprocess(rgb)
            feat = arc_engine(inp).astype(np.float32)  # (1,512)
            person_feats.append(feat[0])
        if person_feats:
            f = np.mean(np.stack(person_feats, axis=0), axis=0)
            f = f / (np.linalg.norm(f) + 1e-8)
            feats.append(f); names.append(person)
    if feats:
...
class SCRFD:
    ...
    def postprocess_scrfd(self, outs_batch, ratio, pad_list):
        ...
        return bboxes_batch, scores_batch, kpss_batch
    ...
    def __call__(self, img_list):
        ...
        start_time = time.time()
        outputs_scrfd, scores_scrfd, kpss_scrfd = self.postprocess_scrfd(outputs, ratio_list, txy_list)
        results_batch = []
        for i,(outputs_scrfd_i,scores_scrfd_i) in enumerate(zip(outputs_scrfd, scores_scrfd)):
            ...
            # attach 5-point landmarks into results if available
            try:
                kps_i = kpss_scrfd[i]
                if kps_i is not None and kps_i.shape[0] == results[0].shape[0]:
                    # flatten 5x2 to 10 columns and concatenate per row
                    kp_flat = kps_i.reshape(kps_i.shape[0], -1)
                    results[0] = np.concatenate([results[0][:,:4], kp_flat, results[0][:,4:]], axis=1)
...
def main(args):
    ...
    if args.arc_bmodel is not None:
        arc_engine = ArcFaceBM(args.arc_bmodel, args.dev_id)
        logging.info(f"Loaded ArcFace bmodel: {args.arc_bmodel}")
        if args.gallery is not None and os.path.isdir(args.gallery):
            logging.info(f"Building gallery from: {args.gallery}")
            gallery_feats, gallery_names = build_gallery_from_dir(scrfd, arc_engine, args.gallery, det_conf=args.conf_thresh)
            logging.info(f"Gallery loaded: {len(gallery_names)} identities")
                ...
                if (len(img_list) == batch_size or cn == len(filenames)) and len(img_list):
                    ...
                    for i, filename in enumerate(filename_list):
                        ...
                        if args.eval:
                            ...
                            # Recognition & draw
                            names = []
                            if arc_engine is not None and gallery_feats is not None and det.shape[0] > 0:
                                for j in range(det.shape[0]):
                                    # try read kps if present
                                    kps = None
                                    if det.shape[1] >= 4+10+2:
                                        kps = det[j, 4:14].reshape(5,2)
                                    # crop & preprocess
                                    if kps is not None:
                                        rgb = align_by_5pts(img_list[i], kps)
                                    else:
                                        x1,y1,x2,y2 = det[j,:4].astype(int)
                                        crop = img_list[i][max(0,y1):max(0,y2), max(0,x1):max(0,x2)]
                                        if crop.size==0:
                                            names.append("Unknown"); continue
                                        rgb = cv2.cvtColor(cv2.resize(crop, (112,112)), cv2.COLOR_BGR2RGB)
                                    inp = arcface_preprocess(rgb)
                                    feat = arc_engine(inp)[0]  # (512,)
                                    if gallery_feats.shape[0] == 0:
                                        names.append("Unknown")
                                    else:
                                        sims = gallery_feats @ feat  # cosine because feats are L2-normalized
                                        k = int(np.argmax(sims))
                                        names.append(gallery_names[k] if sims[k] >= args.sim_thresh else "Unknown")
                            else:
                                names = [""] * det.shape[0]
                            # draw
                            res_img = img_list[i].copy()
                            for j in range(det.shape[0]):
                                x1,y1,x2,y2,score,_ = det[j, :6].astype(np.float32)
                                x1=int(x1); y1=int(y1); x2=int(x2); y2=int(y2)
                                if score < 0.25: 
                                    continue
                                cv2.rectangle(res_img, (x1,y1), (x2,y2), (0,0,255), 2)
                                label = f"{names[j]} {score:.2f}" if names[j] else f"{score:.2f}"
                                cv2.putText(res_img, label, (x1, max(0,y1-6)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2)
                    ...
                    if arc_engine is not None and gallery_feats is not None and det.shape[0] > 0:
                        for j in range(det.shape[0]):
                            ...
                            if det.shape[1] >= 4+10+2:
                                kps = det[j, 4:14].reshape(5,2)
                            if kps is not None:
                                rgb = align_by_5pts(frame_list[i], kps)
                            else:
                                x1,y1,x2,y2 = det[j,:4].astype(int)
                                crop = frame_list[i][max(0,y1):max(0,y2), max(0,x1):max(0,x2)]
                                if crop.size==0:
                                    names.append("Unknown"); continue
                                rgb = cv2.cvtColor(cv2.resize(crop, (112,112)), cv2.COLOR_BGR2RGB)
                            inp = arcface_preprocess(rgb)
                            feat = arc_engine(inp)[0]
                            if gallery_feats.shape[0] == 0:
                                names.append("Unknown")
                            else:
                                sims = gallery_feats @ feat
                                k = int(np.argmax(sims))
                                names.append(gallery_names[k] if sims[k] >= args.sim_thresh else "Unknown")
                    else:
                        names = [""] * det.shape[0]
                    res_frame = frame_list[i].copy()
                    for j in range(det.shape[0]):
                        x1,y1,x2,y2,score,_ = det[j, :6].astype(np.float32)
                        x1=int(x1); y1=int(y1); x2=int(x2); y2=int(y2)
                        ...
                        label = f"{names[j]} {score:.2f}" if names[j] else f"{score:.2f}"
                        cv2.putText(res_frame, label, (x1, max(0,y1-6)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2)
                    out.write(res_frame)
                frame_list.clear()
        cap.release()
        out.release()
...
def argsparser():
    ...
    parser.add_argument('--arc_bmodel', type=str, default=None, help='path to ArcFace bmodel (e.g., arcface_r100_fp32_1b.bmodel)')
    parser.add_argument('--gallery', type=str, default=None, help='gallery dir: subfolders as person names containing images')
    parser.add_argument('--sim_thresh', type=float, default=0.38, help='cosine similarity threshold for recognition')
