...
class SCRFD:
    ...
    def __call__(self, img_list):
        ...
        for i,(outputs_scrfd_i,scores_scrfd_i) in enumerate(zip(outputs_scrfd, scores_scrfd)):
            ...
            # attach 5-point landmarks (flatten 10 dims) into results[0]: [x1,y1,x2,y2, kps(10), score, cls]
            try:
                kps_i = kpss_scrfd[i]
...
def draw_numpy_with_kps(image, det_with_optional_kps):
    """
    det shape can be:
      - [N,6] (x1,y1,x2,y2,score,cls)  -> draw boxes only
      - [N,16] (x1,y1,x2,y2, kps10, score, cls) -> draw boxes + 5pts
    """
    img = image.copy()
    if det_with_optional_kps is None or det_with_optional_kps.shape[0] == 0:
        return img

    for i in range(det_with_optional_kps.shape[0]):
        row = det_with_optional_kps[i].astype(np.float32)
        x1, y1, x2, y2 = row[:4].astype(np.int32)
        score = float(row[-2]) if det_with_optional_kps.shape[1] >= 6 else 0.0
        if score < 0.25:
            continue

        # draw bbox
        cv2.rectangle(img, (x1, y1), (x2, y2), (0, 0, 255), 2)
        cv2.putText(img, f"{score:.2f}", (x1, max(0, y1 - 6)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)

        # draw 5pts if available
        if det_with_optional_kps.shape[1] >= 16:
                ...
                if (len(img_list) == batch_size or cn == len(filenames)) and len(img_list):
                    ...
                    for i, filename in enumerate(filename_list):
                        ...
                        if args.eval:
                            ...
                            # save image
                            res_img = draw_numpy_with_kps(img_list[i], det)
        ...
        while not end_flag:
            ...
            if (len(frame_list) == batch_size or end_flag) and len(frame_list):
                ...
                for i, frame in enumerate(frame_list):
                    ...
                    res_frame = draw_numpy_with_kps(frame_list[i], det)
                    out.write(res_frame)
                frame_list.clear()
        cap.release()
        out.release()
